...
#: Во сколько кеглей обходится самое длинное слово заголовка, с запасом 4%.
#: Замерено в браузере на загруженных шрифтах: польское «Sfotografuj» — 5,28
#: кегля, украинское «Сфотографуйте» — 10,43. Из этого страница сама считает
#: предельный кегль под ширину колонки, поэтому слово не рвётся ни на одной
#: ширине. Правите заголовки — пересчитайте (порядок в README).
FIT = {
    'en': {'h1': 3.67, 'h2': 4.76},
    'pl': {'h1': 5.49, 'h2': 6.82},
    'uk': {'h1': 10.85, 'h2': 10.26},
}
...
def render(c, site):
    ...
    fit = FIT[c['lang']]
    # Только в :root, один раз: в медиазапросах правится сам кегль, а доля
    # слова от кегля у языка одна на все ширины.
    page = page.replace('  --h1-clamp:clamp',
                        '  --h1-fit:%s;\n  --h2-fit:%s;\n  --h1-clamp:clamp'
                        % (fit['h1'], fit['h2']), 1)
